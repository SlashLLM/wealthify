...
function renderAdminLeadEmail(lead) {
  const isBorrowing = lead.source === 'borrowing-capacity-calculator';
  const calcName = isBorrowing ? 'borrowing capacity calculator' : 'refinance calculator';
  const calcNameTitle = isBorrowing ? 'Borrowing capacity calculator' : 'Refinance calculator';

  const title = `New ${calcName} lead`;
  const bodyHtml = `
    ...
    <h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:${BRAND.navy};letter-spacing:-0.5px;line-height:1.2;">
      ${calcNameTitle} — step 1 completed
    </h1>
    <p style="margin:0 0 24px;font-size:14px;color:${BRAND.body};line-height:1.6;">
      Someone started the ${calcName} and submitted their contact details. They have not yet completed step 2 (property address).
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.surface};border-radius:8px;padding:4px 16px;">
      ...
      ${lead.bank_name ? detailRow('Current bank', lead.bank_name) : ''}
  ...
  return {
    subject: `New ${calcName} lead — ${lead.full_name || lead.email}`,
...
function renderClientThankYouEmail(lead) {
  const isBorrowing = lead.source === 'borrowing-capacity-calculator';
  const calcName = isBorrowing ? 'Wealthify borrowing capacity calculator' : 'Wealthify refinance calculator';
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 20px;font-size:15px;color:${BRAND.body};line-height:1.65;">
      Thanks for using the ${calcName}. We have received your details and one of our licensed advisers will reach out to you shortly to walk you through your personalised lender comparison and answer any questions.
    </p>
      ...
      <tr>
        <td>
          ...
          <p style="margin:0;font-size:13.5px;color:${BRAND.body};line-height:1.6;">
            An adviser will be in touch within one business day to confirm your numbers, find your best lender, and explain the process — paperwork included. Free, no obligation.
          </p>
        </td>
      </tr>
    </table>
